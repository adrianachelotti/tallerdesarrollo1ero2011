...
public class ConsultaHistorialGeneral extends Formulario {
	...
	private Model<String> usuarioModel;
	private TextField<String> usuarioTF;
	
    private Model<Integer> cantidadPorPaginaModel;
    private TextField<Integer> cantidadPorPaginaTF;

	
	public ConsultaHistorialGeneral(final Menu menu) {
		super(menu);
		...
		usuarioModel = new Model<String>();
		usuarioTF = new TextField<String>("usuarioTB", usuarioModel,String.class);
		usuarioTF.add(new ActualizacionAjax());
		listContainer.add(usuarioTF);
	...
	private PageableListView crearListView() {
		PageableListView listview = new PageableListView("consultas",
			...
			protected void populateItem(ListItem item) {
				...
				item.add(new Label("usuario", consulta.getUsuario().getUsername()));
	...
	protected void cargarNuevaConsulta() {
		...
		String usuario=usuarioTF.getModelObject();
		...
		while (it.hasNext()) {
			...
			if((mostrar)&&(usuario!=null)&&(!ci.consulta.getUsuario().getUsername().startsWith(usuario))) mostrar=false;
	...
	private void cargarConsultas() {
		consultasMostradas.clear();
		consultas.clear();
		Iterator<Consulta> iteradorConsultas= AdministradorConsultas.obtenerConsultas().iterator();
