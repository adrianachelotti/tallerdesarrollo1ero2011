...
public class AdministradorUsuarios {
	...
	public static boolean modificarPermiso(Permiso permiso) {
		boolean modificado = false;
		if (permiso != null) {
			Session session = HibernateUtil.getSessionFactory().getCurrentSession();
			session.beginTransaction();
	...
	public static List<Permiso> obtenerPermisos() {
		...
		Query query = session.createQuery("from Permiso");
		resultado = query.list();
		session.getTransaction().commit();
		return resultado;
	}

	public static void agregarPermiso(Usuario usuario, Permiso permiso) {
		usuario.agregarPermiso(permiso);
		modificarUsuario(usuario);
		permiso.agregarUsuario(usuario);
		modificarPermiso(permiso);
		
	}

	public static void eliminarPermiso(Usuario usuario, Permiso permiso) {
		usuario.eliminarPermiso(permiso);
		modificarUsuario(usuario);
		permiso.eliminarUsuario(usuario);
		modificarPermiso(permiso);
